 ...
 * 广东省公安厅
 */
public class GdgaAnalysis extends AbstractPubFilter {

    Pattern timePattern = Pattern.compile("时间：(\\d{4})-(\\d{1,2})-(\\d{1,2}) (\\d{1,2}):(\\d{1,2})");

    @Override
    public void preProcessor(NoticeSource noticeSource) {
        RegulatoryNotice regulatoryNotice = noticeSource.getRegulatoryNotice();
        regulatoryNotice.setNoticeOrgan("广东省公安厅");
        regulatoryNotice.setProvince("广东省");
        String value = noticeSource.getValue();
        Document document = Jsoup.parse(value);
        Elements timeEl = document.getElementsByClass("top");
        ...
        if (time != null) {
            try {
                LocalDateTime localDateTime = LocalDateTime.parse(time.replace("时间：",""), dateTimeFormatter);
    ...
    public Element getContentElement(Document document, String url) {
        return document.getElementsByClass("mid text").first();
    }
}
